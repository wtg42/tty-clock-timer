...
既有 workflow 只在 `v*` tag push 時執行，且會建立或更新 GitHub Release。維護者需要一個不發版、不推 tag 的手動 dry-run 路徑，先驗證 AppImage build/package/verify 是否正常。
...
- `tag-driven-appimage-release`: 增加手動 AppImage dry-run 驗證路徑，明確區分 dry-run artifact 與正式 GitHub Release assets。
