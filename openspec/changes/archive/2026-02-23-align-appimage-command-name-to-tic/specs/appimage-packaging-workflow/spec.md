...
#### Scenario: 本機使用共用 fetch 腳本準備 gum
- **WHEN** 維護者在本機執行 AppImage 打包前置流程
- **THEN** 維護者 MUST 可透過 `packaging/appimage/scripts/fetch-gum.sh` 取得 `packaging/tools/gum/linux-x64/gum`
- **AND** 該流程 MUST 驗證下載檔案 checksum，失敗時 MUST 中止
