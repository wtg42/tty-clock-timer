...
- [x] 6.2 等待並確認`appimage-required`與`macos-required`均在GitHub出現且成功
- [x] 6.3 在取得使用者對外部設定變更的明確授權後，為`main`啟用require pull request、兩個required contexts、strict up-to-date、admin enforcement、無bypass actor與0 required approvals
- [x] 6.4 透過GitHub API與PR mergeability狀態驗證缺失/失敗/過期checks會阻止merge，綠燈且最新branch可正常merge
- [x] 6.5 記錄套用後protection設定與rollback命令，並確認未授予PR workflows額外write permission
