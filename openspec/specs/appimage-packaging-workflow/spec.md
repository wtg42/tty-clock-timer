...
- **THEN** 腳本 MUST 先清理專案內 build 產物並重建 core 與 TUI，再產生 AppImage，且不得依賴既有 `stage/usr/bin/tic`
...
- **WHEN** 維護者執行 AppImage verify 腳本
- **THEN** 腳本 MUST 檢查 `AppDir/usr/bin/tic` 存在且可執行
- **AND** 若檢查失敗，verify MUST 回報失敗狀態
